...
The dashboard requires a running agent; opening HTML directly via `file://` is unsupported. No internet, VPN, CDN or remote server is required for local DNS collection.

## Workflow

1. Review **Current DNS Configuration**. Refresh reads current local settings. **Import snapshot or report** accepts schema-v1 DNS snapshots and exported support reports up to 128 MB, including reports exported by earlier versions. It validates nested fields and retains previous valid data on failure. Configured DNS does not prove which resolver handled a query.
...
3. Review **Include speed tests**, checked by default. Cloudflare transfers run only when you start tests: one response capped at 20 MB and one 1 MB synthetic upload per run. Application byte limits exclude protocol overhead and already-buffered transport bytes.
...
- `targets`: up to 20 HTTPS targets with unique IDs, names and a `corporate` flag. The only default target is public example.com. Add your own approved targets locally.
...
- `managedDnsServers`, `legacyDnsServers`: approved inventories. Empty inventories classify nothing; overlapping addresses are rejected. IPv6 is normalized.
...
- `POST /diagnostics/{id}`: configured target ID, `dns-servers`, `ping`, or `portal`. Requires the exact local Origin and `X-LanVibes: run`. Starts probes, not network-setting mutations. Imported HTTPS targets are accepted only through the validated custom diagnostic endpoint; commands are never accepted.
...
```powershell
node --test tests/core.test.mjs tests/startup.test.mjs tests/probes.test.mjs tests/session.test.mjs tests/expired-session.test.mjs tests/review.test.mjs tests/public-source.test.mjs tests/targets.test.mjs
...
See [the Windows 0.8.0 pilot guide](packaging/WINDOWS-PILOT.md). The combined EXE selects x64 or ARM64 automatically and shows an installation wizard by default. Packaged launches run as the signed-in user, use an authenticated random loopback port, open the default browser, and stop through **Quit LanVibes** or 5 minutes without local requests. Active requests prevent idle shutdown. There is no background service or automatic startup. Explicit headless operation retains the configured fixed port and does not auto-exit. Windows/macOS development launches also default to desktop mode. Use `--headless` explicitly with packaged binaries for CLI hosting; `--desktop --no-browser` prints a private launch URL for automated testing and must not be shared. Older release artifacts are historical; use the current LanVibes package.

Windows per-user MSI build scripts and the macOS `.app`/`.pkg` build script are in `packaging/`. `dist/` contains pilot artifacts. macOS payloads can be cross-published, but native installer creation, signing/notarization and device acceptance require a Mac. Test packaged Windows lifecycle with `node tests/desktop.integration.mjs .build/public-0.8.0/publish/win-x64`.
...
- Configured corporate targets `Not applicable` means the selected Home/off-VPN profile did not run those targets. Select Office or VPN and rerun; no VPN state is inferred.
...
Project code is licensed under Apache 2.0; see LICENSE and THIRD_PARTY_NOTICES.md for bundled dependencies. See [public release preparation](PUBLIC-RELEASE.md) for the clean source export, endpoint inventory, and files to exclude from GitHub. Signing and website hosting are optional and currently deferred.

## Import custom applications

Use **Import custom applications** above the environment selector. Accepts a UTF-8 `.txt` file with one HTTPS URL per line, or a JSON array of URL strings or objects containing `name`, `url`, and optional boolean `corporate`. A JSON object with a `targets` array is also accepted. See `examples/custom-applications.json`.

Maximum 10,000 imported applications and 32 MB per file. Credentials, fragments, duplicate URLs and non-HTTPS schemes are rejected. Imports replace only this tab's custom list; configured targets remain. Review the displayed list, choose Office/VPN for corporate entries, then click **Run network tests**. Import itself makes no diagnostic requests. Clear removes the custom list. Reloading discards it; no configuration file is changed.

URL entries default to native DNS, TCP, TLS and HTTP checks. DNS entries run only OS hostname resolution. Cancellation and the progress bar apply to both; the progress bar names the current application and URL/hostname. Results include the application name and URL in support reports; avoid URLs containing sensitive query parameters. Results from a previous run remain exportable until you start another run. Custom lists are independent of support-report imports.

The agent independently validates each `/diagnostics/custom` JSON POST, caps its request size, requires the same-origin custom header and desktop session, and shares the existing diagnostic concurrency limit and deadline. Internal targets are permitted intentionally for local network diagnostics. Headless mode retains its existing local-process access boundary.
Use `{"name":"DNS lookup","dns":"portal.corp.example","corporate":true}` for DNS-only resolution, or add `"mode":"dns"` to a URL entry. `mode` defaults to `full` for URLs and `dns` for hostnames. Supply either `url` or `dns`, not both. Markdown links pasted into the URL field are normalized to their destination. Errors identify the entry number and retain the previous valid list.

To restrict an entire run to DNS, select **DNS resolution only**. This runs imported applications only, skips baseline, public-provider, inventory and speed tests, and performs no TCP/TLS/HTTP probes to the application. Choose Office or VPN to include entries marked corporate; Home skips them. OS DNS uses the workstation resolver and cache; this does not enumerate every DNS record type or prove which resolver answered.

Import displays pending test rows immediately. Tests start only after clicking Run network tests. Large lists and saved reports use pages of 100 results, while the import preview shows the first 100 entries. All collected results are retained for export. Support reports allow up to 128 MB, 20,000 results and 300,000 stage observations; exceptionally large details can still exceed the byte limit. Older app versions may reject these larger reports. Runs remain sequential and cancellable; large lists can take considerable time when targets time out.
Custom application and DNS-only results share one collapsed summary panel showing counts of succeeded, failed, inconclusive, skipped, cancelled, running and not-run applications. Expand the panel to view individual results and stage details, paged in groups of 100. Arriving results update counts without opening the panel. Keyboard users can toggle the summary with Enter or Space.


Before user-triggered refresh, inspection or test runs, the dashboard checks the local agent. If the session is unavailable, it preserves displayed data for export, disables new diagnostics, and asks the user to reopen LanVibes. Idle shutdown is described as a possible cause, not inferred as certain from a fetch failure. These checks are on demand; no background heartbeat keeps the app alive.


## Find, retry and compare results

Search application names or hostnames and select a status filter. Filters change displayed cards only: the custom summary, export and retries include all applicable results. Filters are reset when a run or retry starts and disabled while it runs. Pagination still limits custom details to 100 matching rows.

**Retry failed / inconclusive** re-executes only matching tests in the current configuration and mode whose previous status was Fail or Inconclusive. Successful, skipped and cancelled results remain intact. It uses the current environment and speed-test choices; filters do not limit retries. Progress counts just the retried checks. Each new result records a timestamp, preserved in support-report exports. A retry report therefore contains observations from different times.

**Compare two runs** accepts two unredacted support reports, or an earlier report plus a copy of the current results. It performs no network tests and stores nothing outside this tab. Results match by exact test name (custom names include destination and mode); renamed tests appear as added/missing and duplicate names are flagged as ambiguous. Comparison highlights overall/stage statuses and OS DNS address changes, ignoring answer order and timing differences. DNS configuration comparison includes resolver/server ordering, suffixes, scopes and NRPT; expand its details to inspect earlier and later values. Missing snapshots are explicitly reported as unavailable. Partial runs and differing profiles can legitimately produce skipped, cancelled or missing results. Export timestamps identify the files, not necessarily the time every check ran.

### Simple and Advanced views

The dashboard opens in a simple view with Run/Stop/Quit controls, location selection, custom application import, report export, progress, current DNS configuration, and a combined results summary. Enable **Advanced** to inspect individual results, change test options, retry checks, or import and compare saved reports. Changing the view does not run tests or change test coverage. Speed tests remain enabled by default; the visible scope note reflects any changes to advanced test options. Reports always include all collected results and details.
